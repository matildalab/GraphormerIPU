...
def collator(items, max_node=512, multi_hop_max_dist=20, rel_pos_max = 20):
    ...
    returningTuple = (torch.LongTensor(idxs), attn_bias, attn_edge_type, rel_pos, all_rel_pos_3d_1, in_degree, out_degree, x, edge_input, y)
    return Batch(idx=torch.LongTensor(idxs),attn_bias=attn_bias,attn_edge_type=attn_edge_type,
                rel_pos=rel_pos,all_rel_pos_3d_1=all_rel_pos_3d_1,in_degree=in_degree,
                out_degree=out_degree,x=x,edge_input=edge_input,y=y,)
